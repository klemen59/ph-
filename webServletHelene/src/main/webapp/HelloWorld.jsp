		...
		<table width=300px id="myTable"
			...
			<thead>
				...
				<tr>
					...
					<!-- <th scope="col"><input type="text" id="myInput"
						onkeyup="myFunction()" placeholder="Recherche par Nom"
						title="Type in a name"></th> -->
				</tr>
			</thead>
			<c:forEach items="${fiche }" var="f">
				...
				<tbody>
					<tr>
						...
						<td>${f.patient.pId }</td>
						<td>${f.patient.nom }</td>
						<td>${f.patient.prenom }</td>
						<td>${f.dateRdv }</td>
						 <td><form action="delete" method="post">
								<input type="hidden" name="deleteId" value="${p.pid}"> <input
									type="submit" value="supprimer" title="delete" id="delete"
